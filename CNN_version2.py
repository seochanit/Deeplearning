...
class CnnModel:
    ...
    def create_model(verbose=True):
        ...
        # Block 1
        x = Conv2D(filters=64, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(input_tensor)
        x = Conv2D(filters=64, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = BatchNormalization()(x)
        
        # Block 2
        x = Conv2D(filters=128, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = Conv2D(filters=128, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = BatchNormalization()(x)
        
        # Block 3
        x = Conv2D(filters=256, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = Conv2D(filters=256, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = BatchNormalization()(x)
        
        # Block 4
        x = Conv2D(filters=512, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = Conv2D(filters=512, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu', kernel_regularizer=l2(0.001))(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = BatchNormalization()(x)
        ...
        x = Dense(units=512, activation='relu', kernel_regularizer=l2(0.001))(x)
        x = Dropout(rate=0.3)(x)
